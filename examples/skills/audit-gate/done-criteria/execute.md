...
- **verification**:
  1. List all task IDs from the implementation plan document
...
- **fail_diagnosis_hint**: Identify task IDs without code changes and check the corresponding tasks in the implementation plan document. Determine whether it is a missing implementation or a documentation-only task whose changes do not appear in git diff
...
- **verify_type**: inspection
- **verification**:
  1. List all test cases from the implementation plan document's test case section
...
- **depends_on_artifacts**: [docs/plans/, tests/]

### EXECUTE-06: Implementation respects component boundaries
- **severity**: quality
- **verify_type**: inspection
- **verification**:
  1. Read the component boundaries (module partitioning, layer architecture) defined in the project's architecture documentation
...
- **fail_diagnosis_hint**: Identify the boundary-violating import/require statements and cross-reference with the project's architecture documentation. Check for cases where dependencies should go through an interface layer instead of direct references
- **depends_on_artifacts**: [docs/plans/, src/]

### EXECUTE-07: End-to-end traceability from requirements to plan to implementation
- **severity**: blocker
- **verify_type**: inspection
- **verification**:
  1. Enumerate all requirements from the requirements source document
  2. Identify the implementation plan document task ID corresponding to each requirement
...
- **verify_type**: inspection
- **verification**:
  1. Enumerate all requirements from the requirements source document and assign IDs
  2. Enumerate the impacted files/modules from the impact analysis
...
- **depends_on_artifacts**: [docs/plans/, tests/, src/]
