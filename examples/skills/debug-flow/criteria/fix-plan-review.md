...
- **fail_diagnosis_hint**: Identify unresolved finding IDs and check the relevant sections of the fix plan document. If fixes are not reflected, verify that the /implementation-review feedback loop completed
...
- **verification**:
  ...
  3. Verify that task completion conditions in the fix plan document do not deviate from Fix Strategy items (no additional features not in the RCA Report, no ignored Fix Strategy constraints)
...
- **fail_diagnosis_hint**: Compare the inconsistent entries side-by-side between the RCA Report and fix plan documents. Check for cases where a review fix updated only one document. Trace the cause using `git log --oneline -- docs/plans/`
...
- **verify_type**: inspection
- **verification**:
  1. Extract the completion condition (done condition / acceptance criteria) from each task in the fix plan document
