...
### FIX-PLAN-REVIEW-01: Review artifact (findings-spec.json) exists
- **severity**: quality
- **verify_type**: automated
- **verification**:
  1. Read `belt-agent status` and locate the artifact `findings-spec` resolved_path
...
- **depends_on_artifacts**: [findings-spec]
...
- **depends_on_artifacts**: [findings-spec, docs/features/*/fix-plan.md]

### FIX-PLAN-REVIEW-04: Every finding has a user-recorded disposition
- **severity**: blocker
- **verify_type**: inspection
- **verification**:
  1. Read `belt-agent status` and locate the artifact `findings-spec` resolved_path
  2. Enumerate all findings at that resolved_path
  3. For each finding, verify a disposition from the batched triage has been recorded by the user (applied or explicitly rejected)
  4. List findings with no recorded disposition
- **pass_condition**: Step 4 list is empty — every finding in findings-spec.json has a user-recorded disposition from the batched triage (applied or explicitly rejected)
- **fail_diagnosis_hint**: Return to the batched triage prompt in the main context. The user must apply or explicitly reject each finding; do NOT let the orchestrator auto-resolve
- **depends_on_artifacts**: [findings-spec]

### FIX-PLAN-REVIEW-05: Applied changes are confined to the fix-plan document(s)
- **severity**: blocker
- **verify_type**: automated
- **verification**:
...
### FIX-PLAN-REVIEW-06: Internal markdown links in the fix-plan still resolve
- **severity**: quality
- **verify_type**: inspection
- **verification**:
